    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if state not in explored:
                    explored.add(state)
            if not hitsWall:
                newCorners = copy.copy(state[1])
                nextMove = (nextx, nexty)
                if nextMove in newCorners:
                    newCorners = list(state[1])
                    newCorners.remove(nextMove)
                succState = ((nextMove, tuple(newCorners)), action, 1)
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    unvisitedFood = list(foodGrid.asList())
    visitedFood = set()
